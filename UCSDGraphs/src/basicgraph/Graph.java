...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		List<Integer> degreeSequToReturn = new ArrayList<Integer>();
		//degree is sum of its indegree and outdegree. largest to smallest.
		
		for(int i=0; i < this.getNumVertices(); i++) //for every vertice
		{
			//System.out.println(i);
			List<Integer> inDegreeList = new ArrayList<Integer>();
			inDegreeList = this.getInNeighbors(i); //get in-neighbors list.
			int inDegree= inDegreeList.size(); //look its size.
			
			//get all out-degree neighbors.
			int outDegree=this.getNeighbors(i).size();
			
			int degreeSeq=inDegree + outDegree; //find degree for that vertice.
			
			degreeSequToReturn.add(degreeSeq);
				
		}
		//sort the Arraylist in decreasing order.
		Collections.sort(degreeSequToReturn, Collections.reverseOrder());
		
		return degreeSequToReturn;
	}
