...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		int[][] matrix;
		matrix = adjMatrix;
		int aRows=this.getNumVertices();
		int aColumns=this.getNumVertices();
		int bRows=this.getNumVertices();
		int bColumns=this.getNumVertices();
		
		if (aColumns != bRows) {
            throw new IllegalArgumentException("A:Rows: " + aColumns + " did not match B:Columns " + bRows + ".");
        }
		
		//adding zeros to solution matrix.
			int solutionmatrix[][]= new int[aRows][bColumns];
			...
			for (int i = 0; i < aRows; i++) { // aRow
	            for (int j = 0; j < bColumns; j++) { // bColumn
	                for (int k = 0; k < aColumns; k++) { // aColumn
	                    solutionmatrix[i][j] += matrix[i][k] * matrix[k][j];
	                }
	            }
	        }
		//creating the twohop neighbors list.	
			for (int i = 0; i < solutionmatrix[0].length; i ++) { //row is fixed check columns.
				for (int j=0; j< solutionmatrix[v][i]; j ++) {
					twohop.add(i);
				}
			}
		...
		//then return list of neighbors of V.
		return twohop;
	}
