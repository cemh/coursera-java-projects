...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 ...
		 //get method returns object value:  in this case onehoop is the arraylist of V.(neighbors of V)
		 List<Integer> oneHop= adjListsMap.get(v);
		 //loop through one Hop and get the neighbors of each.
		 
		 List<Integer> twoHopbuffer= new ArrayList<Integer>();
		 
		 //look at all neighbors.
		 for(int i=0; i < oneHop.size();i++)
		 {
			 twoHop = adjListsMap.get(oneHop.get(i));
			// System.out.println(twoHop);
			 twoHopbuffer.addAll(twoHop);
			// System.out.println(twoHopbuffer);
			 //Merging two arraylists together.
