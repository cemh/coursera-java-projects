...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		// TODO: Implement this method. add one letter.  
		for(int index = 0; index < s.length()+1; index++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
				...
				x=x.substring(0, index)+(char)charCode + x.substring(index, x.length());
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		// TODO: Implement this method. remove one letter.
		for(int index = 0; index < s.length(); index++){
		...
		while (!queue.isEmpty() && count<numSuggestions){			
				//remove the first string from the queue
	   		  ...
	   		  // count = neighbors.size();
	   		   // for each neighbors look.
	   		   for (String n: neighbors)  
	   		   {
	   			  
	   			   //if not visited add to visited list and add to the back of the queue.
